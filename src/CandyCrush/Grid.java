...
public class Grid {
	...
	public int[] findMaximum(Candy[] candies) { 
        ...
        int startIndex 		= 0;	//starting index of longest sequence so far

        for (int i = 1; i < candies.length; i++) {

			if(candies[i-1] == null || candies[i] == null) {
				continue;
			}
	...
	*/
	...
	public Candy[] getColumn(int col) {
        ...
        Candy[] result = new Candy[grid.length];
		for(int i = 0; i < grid.length; i++){
			result[i] = grid[i][col];
	...
	public int inAColumn() {
		for(int col = 0; col < grid[0].length; col++){
			int[] temp = findMaximum(getColumn(col));
			if(temp[0] >= 3){
				return col;
			}
		}
